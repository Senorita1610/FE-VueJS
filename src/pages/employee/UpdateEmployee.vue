<template>
    <Dialog v-model:visible="userStore.updateEmployeeDialog" :style="{ width: '450px' }" header="Update a employee"
        ...
        <form @submit.prevent="submitForm">
            ...
            <div class="field">
                ...
                <Calendar id="birthday" v-model="v$.joinDate.$model" dateFormat="dd-mm-yy" showIcon
...
const closeDialog = () => {
    userStore.updateEmployeeDialog = false;
    accountStore.submitted = false;
    // resetForm();
};

watch(() => userStore.employee, (newVal) => {
    if (newVal) {
        state.employeeId = newVal.employeeId;
        state.address = newVal.address;
        state.birthday = newVal.birthday;
        state.gender = newVal.gender;
        state.joinDate = newVal.joinDate;
...
const submitForm = async () => {
    ...
    if (!v$.value.$invalid) {
        try {
            ...
            await userStore.updateEmployee(state);
            Swal.fire({
                title: userStore.message ? 'Error' : 'Success',
                text: userStore.message ? userStore.message : 'Update employee successfully!',
